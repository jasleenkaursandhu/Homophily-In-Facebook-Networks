...
# Select the attribute to analyze (e.g., "gender")
attribute_index = 26  # Adjust this index for the "gender" attribute
...
if len(matching_edges) + len(non_matching_edges) > 0:
    ...
    print(f"Proportion of matching gender attribute edges: {proportion_matching:.2f}")
else:
    print("No edges with attributes found.")

# Create a visualization of the network, differentiating matching and non-matching gender attribute edges with colors
...
plt.axis("off")
plt.savefig("images/network_with_gender.png")
plt.show()
