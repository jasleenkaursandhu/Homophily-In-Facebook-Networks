...
# Select the attribute to analyze (e.g., "education")
attribute_index = 2  
...
if len(matching_edges) + len(non_matching_edges) > 0:
    ...
    print(f"Proportion of matching education attribute edges: {proportion_matching:.2f}")
else:
    print("No edges with attributes found.")

# Create a visualization of the network, differentiating matching and non-matching education attribute edges with colors
...
plt.axis("off")
plt.savefig("images/network_with_education.png")
plt.show()
